...
@router.get("/topics", response_model=list[schemas.TopicBaseSchema])
...
@router.post("/topics", response_model=schemas.TopicCreate)
async def create_topic(topic: schemas.TopicCreate,
                       db: AsyncSession = Depends(get_db),):
    return await crud.create_topic(db=db, topic_data=topic)


@router.put("/topics/{topic_id}", response_model=schemas.TopicUpdate)
...
@router.delete("/topics/{topic_id}", response_model=schemas.TopicDelete)
